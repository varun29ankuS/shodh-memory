...
impl GlinerNer {
    ...
    fn decode_bio(
        ...
        // Channel semantics (verified against the fp32 reference model):
        //   ch0 = span START score, ch1 = span END score, ch2 = word INSIDE-span.
        // A span opens at a word with start>=thr, runs while inside>=thr, and
        // closes at the first word with end>=thr. The previous decode treated
        // ch1 as "inside", which silently truncated every entity longer than
        // two words ("Synergy Marine Group" -> "Synergy") — start fired, then
        // ch1(end) was ~0 mid-span, so extension stopped at word one.
        ...
        for c in 0..num_classes {
            ...
            while w < num_words {
                ...
                // Scan forward for the closing END while INSIDE holds.
                let mut found_end: Option<usize> = None;
                let mut k = w;
                while k < num_words && k < w + MAX_SPAN_WORDS && at(2, k, c) >= threshold {
                    if at(1, k, c) >= threshold {
                        found_end = Some(k);
                        break;
                    }
                    k += 1;
                }
                let Some(end_w) = found_end else {
                    // Start without a reachable end: not a well-formed span —
                    // emitting it would recreate the fragment-entity class.
                    w += 1;
                    continue;
                };
                let cs = words[w].1;
                ...
                out.push(GlinerEntity {
                    ...
                    score: (at(0, w, c) + at(1, end_w, c)) / 2.0,
                });
                w = end_w + 1;
            }
        }

        // Resolve overlaps: longest span wins at each start; among identical
        // spans keep the HIGHEST-SCORING label (class order must never decide —
        // with a broad zero-shot schema several labels can clear threshold on
        // the same span, and the best one is the model's actual answer).
        out.sort_by(|a, b| {
            a.start
                .cmp(&b.start)

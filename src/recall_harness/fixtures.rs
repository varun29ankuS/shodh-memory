...
/// Query categories. The first six are the L1 smoke suite's; `SingleHop` and
/// `OpenDomain` additionally appear in the held-out LoCoMo suite (which reuses
/// this harness for an LLM-free recall@k generalization check). Only the smoke
/// six are balance-validated — see [`SmokeCategory::ALL`].
...
pub enum SmokeCategory {
    ...
    SingleHop,
    OpenDomain,
    /// Directional suite: "Who did A <verb> ...?", asking for the object.
    DirObject,
    /// Directional suite: "Who <verbed> B ...?", asking for the subject.
    DirSubject,
    /// Directional suite: the memories are passive ("B was <verbed> by A") and the
    /// questions active.
    DirPassive,
}

impl SmokeCategory {
    /// The smoke suite's categories — the set that `validate_smoke_suite`
    /// balance-checks. LoCoMo-only categories are intentionally excluded.
...
/// Path to the held-out LoCoMo recall corpus (dialogue turns) and cases
/// (questions with gold evidence dia-ids). Generated by
/// `benchmarks/locomo_to_harness.py` from snap-research/locomo. Used for an
/// LLM-free recall@k generalization check via `recall-eval --suite locomo`.
pub const LOCOMO_CORPUS_PATH: &str = "tests/recall/corpora/locomo.jsonl";
pub const LOCOMO_CASES_PATH: &str = "tests/recall/locomo_cases.jsonl";

/// The frozen LoCoMo-100 fast CI gate (replaces the synthetic 108-case smoke
/// suite). 100 category-balanced real LoCoMo cases against a single
/// conversation's 629-turn corpus — small enough to gate a PR in ~1-2 min,
/// real enough to be meaningful. Generated by `benchmarks/build_locomo_gate.py`;
/// run via `recall-eval --suite locomo-gate --baseline
/// tests/recall/locomo-gate-baseline.json`.
pub const LOCOMO_GATE_CORPUS_PATH: &str = "tests/recall/corpora/locomo-gate.jsonl";
pub const LOCOMO_GATE_CASES_PATH: &str = "tests/recall/locomo-gate-cases.jsonl";

/// Who did what to whom. Every gold memory has a twin of exactly the same words
/// with the roles swapped, so no lexical signal separates them and a system that
/// ignores direction scores 50% p@1. Generated by
/// `benchmarks/build_directional_suite.py`.
pub const DIRECTIONAL_CORPUS_PATH: &str = "tests/recall/corpora/directional.jsonl";
pub const DIRECTIONAL_CASES_PATH: &str = "tests/recall/directional_cases.jsonl";
...
mod tests {
    ...
    /// The directional suite measures direction only if every gold has a twin made of
    /// the same words with the roles swapped. Otherwise a lexical cue could answer the
    /// question. This pins that property on the checked-in files.
    #[test]
    fn directional_fixtures_give_every_gold_a_same_words_twin() {
        use std::collections::HashMap;
        let corpus =
            load_corpus(&manifest_path(DIRECTIONAL_CORPUS_PATH)).expect("directional corpus loads");
        let cases = load_smoke_cases(&manifest_path(DIRECTIONAL_CASES_PATH))
            .expect("directional cases load");
        validate_structure(&corpus, &cases).expect("directional suite is structurally valid");
        ...
        let by_id: HashMap<&str, &CorpusItem> = corpus.iter().map(|c| (c.id.as_str(), c)).collect();
        for case in &cases {
            assert!(
                matches!(
                    case.category,
                    SmokeCategory::DirObject
                        | SmokeCategory::DirSubject
                        | SmokeCategory::DirPassive
                ),
                "case {} has category {:?}",
                case.id,
                case.category
            ...
            assert_eq!(
                case.relevant.len(),
                1,
                "case {} must have one gold",
                case.id
            );
            let gold = by_id[case.relevant[0].corpus_item_id.as_str()];
            let twins: Vec<&&CorpusItem> = by_id
                .values()
                .filter(|c| c.id != gold.id && bag(&c.content) == bag(&gold.content))
                .collect();
            assert_eq!(
                twins.len(),
                1,
                "gold {} of case {} must have exactly one same-words twin",
                gold.id,
                case.id
            );
            assert_ne!(
                twins[0].content, gold.content,
                "twin must differ in word order"
            );
        }
    }

    #[test]
    fn locomo_fixtures_load_and_every_evidence_resolves() {
        use std::collections::HashSet;
        let corpus = load_corpus(&manifest_path(LOCOMO_CORPUS_PATH)).expect("locomo corpus loads");
        let cases = load_smoke_cases(&manifest_path(LOCOMO_CASES_PATH)).expect("locomo cases load");

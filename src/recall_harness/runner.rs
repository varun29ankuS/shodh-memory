...
fn run_one_pass(
    ...
    // Cross-runner stage fingerprints (`SHODH_STAGE_EXPORT=<jsonl path>`).
    //
    // Four gate runs of one tree gave three different per-case result sets
    // across ubuntu-latest runners while each run was byte-stable over five
    // repeats, and one case's gold entered the 100-deep pool on one runner
    // only -- so the divergence is upstream of the reranker. The repeat
    // determinism check compares stages between passes of ONE process; this
    // writes the same stages, plus the ingest-side ones it cannot see
    // (embedding bytes, NER spans, the graph's node and edge sets), so two
    // processes on two machines can be diffed stage by stage. Off unless the
    // variable is set; Full mode only.
    ...
    if let Some(path) = &stage_export {
        if !stage_lines.is_empty() {
            ...
            let mut f = std::fs::OpenOptions::new()
                ...
                .append(true)
                .open(path)
                .with_context(|| format!("opening stage export {}", path.display()))?;

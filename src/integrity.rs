...
//! `Memory` through it. `read_sho_envelope` now refuses to hand a failed
//! checksum to that chain, but nothing about the chain itself changed: a
//! pre-envelope record, or any value that never carried SHO magic in the first
//! place, still reaches it and can still come back as a fabricated `Memory`
//! with no warning past the first one.
//!
//! A scrub that asks "does every record decode?" reports that population as
//! perfectly healthy. This module therefore classifies into five states, not
//! two:
//!
//! | state | meaning |
//! |---|---|
...
//! | [`RecordClass::ChecksumMismatch`] | the envelope is present and damaged: a failed CRC32, or a length no envelope can have |
//!
//! `Implausible` is the point of this module. The other four are states the
//! read path can also reach; that one is invisible to it by construction.
//!
//! # Relationship to [`crate::serialization::ShoEnvelope`]
//!
//! The envelope type classifies *bytes*, this module classifies *records*, and
//! the mapping between them is spelled out in full on [`decode_memory_value`].
//! The scrub used to carry a private copy of the envelope parser because
//! `unwrap_sho` collapsed "no envelope" and "corrupt envelope" into one `None`;
//! that copy is gone, and there is now one envelope reader in the process.
//!
...
//! `fill_cache(false)` and classifies values without ever writing one back.
...
pub enum RecordClass {
    ...
    Undecodable,
    /// The SHO envelope is present but damaged: its stored CRC32 does not
    /// match the payload, or the record is shorter than the smallest possible
    /// envelope. Either way the bytes on disk are not the bytes that were
    /// written, and neither may be handed to a fallback decoder. The two are
    /// kept apart in `decode_paths` (`sho_crc_mismatch` / `sho_truncated`).
    ChecksumMismatch,
}
...
enum DecodeOutcome<T> {
    ...
    /// The envelope is present but damaged: its CRC32 disagrees with its
    /// payload, or it is shorter than the smallest possible envelope. Both
    /// carry a decode-path name so the report keeps them apart.
    EnvelopeCorrupt(String, &'static str),
}

/// Classify one memory value at the wire level.
///
/// # Where the scrub's four states and the envelope's four states meet
///
/// [`crate::serialization::read_sho_envelope`] answers a question about *bytes*
/// -- is there an envelope, and do the bytes match their own checksum. This
/// module answers a question about *records* -- can the writer have produced
/// this. They are different questions, so neither type subsumes the other, and
/// the merge kept both rather than collapsing one into the other. What the
/// merge did remove is the scrub's own private envelope parser (`split_sho`),
/// which existed only because `unwrap_sho` used to collapse "no envelope" and
/// "corrupt envelope" into a single `None`. `read_sho_envelope` now draws
/// exactly the distinction the scrub had to draw for itself, so the duplicate
/// is gone and there is one envelope reader in the process.
///
/// The mapping, total on both sides:
///
/// | `ShoEnvelope` | `RecordClass` | why |
/// |---|---|---|
/// | `Absent` | decided by the legacy chain: `Legacy` or `Undecodable` | a pre-cutover record is legitimate; only the decode result can judge it |
/// | `Truncated` | `ChecksumMismatch` | magic present, envelope impossible -- damaged bytes, and the read path errors on it too |
/// | `ChecksumMismatch` | `ChecksumMismatch` | the bytes on disk are not the bytes that were written |
/// | `Valid { version: 2 }` | `Clean` / `Legacy` / `Implausible` / `Undecodable` | the plausibility checks decide |
/// | `Valid { version: 1 }` | `Legacy` / `Implausible` / `Undecodable` | bincode generation: readable, aging |
/// | `Valid { version: other }` | `Undecodable` | a format this build predates; the legacy chain could only fabricate from it |
///
/// The two corrupt envelope states share one `RecordClass` because the verdict
/// rule treats them identically -- both mean "damaged, do not fall back" -- but
/// they keep separate `decode_paths` entries (`sho_truncated`,
/// `sho_crc_mismatch`) so the report can still tell a bit flip from a truncated
/// write. A fifth `RecordClass` would split a class no rule distinguishes,
/// which is duplication in the other direction.
///
/// # The scrub decodes through the decoder that serves reads
///
/// The postcard branch calls
/// [`crate::memory::storage::decode_postcard_memory`], not `decode_raw_compat`
/// directly. That function carries the `NerWireGeneration::PreFineLabel` retry:
/// `NerEntityRecord` gained a mid-payload field in July, so every pre-July
/// record with a non-empty `ner_entities` desynchronises the current decoder.
/// Bypassing the retry would make the scrub report a population `Undecodable`
/// that the running server reads without difficulty -- a scrub measuring a
/// decoder nobody uses, which is the exact failure these docs warn about for
/// the legacy chain.
fn decode_memory_value(value: &[u8]) -> DecodeOutcome<Memory> {
    ...
    match crate::serialization::read_sho_envelope(value) {
        ShoEnvelope::Truncated { len } => DecodeOutcome::EnvelopeCorrupt(
            format!(
                "SHO magic present but the record is {len} bytes, shorter than the \
                 smallest possible envelope; there is no payload to checksum"
            ),
            "sho_truncated",
        ),
        ShoEnvelope::ChecksumMismatch { stored, computed } => DecodeOutcome::EnvelopeCorrupt(
            format!(
                "stored CRC32 {stored:08x} does not match computed {computed:08x}; \
        ...
        ShoEnvelope::Valid {
            ...
            payload,
        } => match crate::memory::storage::decode_postcard_memory(payload) {
            Ok((m, false)) => DecodeOutcome::Current(m, "sho_v2_postcard"),
            // Decoded only after supplying defaults for trailing fields the
            // record predates, or after re-running under the pre-fine-label NER
            // wire generation. Readable, but written against an older schema --
            // exactly the population a positional change breaks next.
            Ok((m, true)) => DecodeOutcome::Legacy(m, "sho_v2_postcard_compat".to_string()),
        ...
        ShoEnvelope::Valid { version, .. } => DecodeOutcome::Failed(
            format!("unknown SHO envelope version {version}"),
            "sho_unknown_version",
        ),
        ShoEnvelope::Absent => {

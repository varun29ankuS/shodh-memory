...
pub fn default_endpoint() -> PathBuf {
    #[cfg(windows)]
    {
        PathBuf::from(r"\\.\pipe\shodh-memory")
    }
...
impl IpcClient {
    ...
    pub async fn post<T: Serialize, R: DeserializeOwned>(
        ...
        self.request("POST", path, body, DEFAULT_CLIENT_TIMEOUT)
            .await
    }
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    let response_id = serde_json::from_slice::<Value>(&frame)
        .ok()
    ...
    S: AsyncWrite + Unpin,
{
    let write = async {
        match serde_json::to_vec(&response) {
            Ok(mut encoded) if encoded.len() < MAX_FRAME_BYTES => {
                encoded.push(b'\n');
            ...
            Ok(_) => tracing::error!("Shodh IPC response exceeded its frame limit"),
            Err(error) => tracing::error!("Failed to serialize Shodh IPC response: {error}"),
        }
    };
        ...
        if let Some(newline) = buffer[..count].iter().position(|byte| *byte == b'\n') {
            ...
            if newline + 1 != count {
                return Err(FrameError {
                    ...
                    message: "IPC accepts exactly one newline-delimited request per connection"
                        .to_string(),
                });
            }
...
mod platform {
    ...
    pub async fn bind(path: &Path) -> Result<Listener, String> {
        ...
        {
            return Err(
                "IPC directory must be a non-symlink directory owned by the current user with mode 0700"
                    .to_string(),
            );
        }
            ...
            tokio::select! {
                ...
                accepted = inner.accept() => {
                    let (stream, _) = match accepted {
                        ...
                        Err(error) => break Some(format!("failed to accept IPC connection: {error}")),
                    };
    ...
    pub async fn serve(
        ...
        let terminal_error = loop {
            ...
            tokio::select! {
                ...
                result = next.connect() => {
                    if let Err(error) = result {
                        break Some(format!("failed to accept IPC pipe: {error}"));
                    }
                    let connected = next;
                    let replacement = create_server(&endpoint, false);
                    let connection_router = router.clone();
                    connections.spawn(async move {
                        let _permit = permit;
...
mod tests {
    ...
    async fn health_round_trip_uses_versioned_envelope() {
        ...
        let listener = LocalIpcServer::bind(endpoint.clone()).await.unwrap();
        ...
        let client = IpcClient::new(endpoint, "not-needed-for-health".to_string());
    ...
    #[test]
    fn streaming_routes_are_excluded_from_finite_ipc() {
        assert!(STREAMING_PATHS.contains(&"/api/stream"));
        assert!(STREAMING_PATHS.contains(&"/api/events/sse"));
        assert!(PUBLIC_API_PATHS.contains(&"/api/context/status"));
    }
    ...
    async fn windows_pipe_dacl_is_protected_and_user_scoped() {
        ...
        let listener = LocalIpcServer::bind(endpoint).await.unwrap();
    ...
    fn test_endpoint(label: &str) -> PathBuf {
        ...
        {
            std::env::temp_dir()
                .join(format!("shodh-memory-test-{}", Uuid::new_v4()))

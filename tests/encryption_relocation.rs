//! Contract: a primary record relocated to a different id must fail to decrypt
//! (identity binding — the record's key is the AEAD associated data). Own test
//! binary so the process-global keystore crypto is isolated.
...
fn relocated_record_is_rejected_by_identity_binding() {
    ...
    let db_path = temp.path().join("storage");
    {
        ...
        let b_bytes = db
            ...
            .expect("B present");
        assert!(
            shodh_memory::keystore::is_encrypted_record(&b_bytes),
